...
import { OnOffLightDevice, OnOffLightRequirements, OnOffPlugInUnitDevice } from "@matter/main/devices";
...
async function main() {
    ...
    const OnOffPlugInUnitDeviceWithLevel = OnOffPlugInUnitDevice.with(OnOffLightRequirements.LevelControlServer);
    const OnOffLightDeviceWithLevel = OnOffLightDevice.with(OnOffLightRequirements.LevelControlServer);

    //const endpoint = new Endpoint(isSocket ? OnOffPlugInUnitDevice.with(OnOffLightRequirements.LevelControlServer) : OnOffLightDevice.with(OnOffLightRequirements.LevelControlServer), { id: "onoff" });
    const endpoint = new Endpoint(isSocket ? OnOffPlugInUnitDeviceWithLevel : OnOffLightDeviceWithLevel, { id: "onoff" });
    ...
    endpoint.events.levelControl.currentLevel$Changed.on(value => {
        console.log(`!@#$ Level is now ${value}%`);
    });
